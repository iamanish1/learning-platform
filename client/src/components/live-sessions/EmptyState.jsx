...
const EmptyState = memo(({ filters, onClearFilters }) => {
  ...
  const getMessage = () => {
    if (hasActiveFilters) {
      if (filters.search) {
        return `No sessions found matching "${filters.search}"`;
      }
      if (filters.status !== 'all') {
        return `No ${filters.status} sessions available`;
      }
      if (filters.type !== 'all') {
        return `No ${filters.type} sessions available`;
      }
      if (filters.category !== 'all') {
        return `No sessions found in "${filters.category}" category`;
      }
  ...
  const getSuggestions = () => {
    if (hasActiveFilters) {
      return [
        ...
        'Clear all filters to see all sessions',
    ...
    >
      <div className="max-w-md text-center">
        ...
        <h3 className="text-2xl font-bold text-gray-900 mb-2">
          {hasActiveFilters ? 'No Results Found' : 'No Sessions Available'}
        </h3>
