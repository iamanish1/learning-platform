...
import { Search, FileText, Code, BookOpen, MessageSquare, Filter } from 'lucide-react';

/**
 * BlogEmptyState component with contextual messaging
 * @param {Object} props
 ...
 */
const BlogEmptyState = memo(({ filters, onClearFilters, isAuthenticated = false }) => {
  ...
  const getIcon = () => {
    ...
    if (filters.type === 'project') return Code;
    if (filters.type === 'documentation') return BookOpen;
    if (filters.type === 'opinion') return MessageSquare;
    return FileText;
  };
  ...
  const getMessage = () => {
    if (hasActiveFilters) {
      if (filters.search) {
        return `No content found matching "${filters.search}"`;
      }
      if (filters.type !== 'all') {
        return `No ${filters.type} content available`;
      }
      if (filters.category !== 'all') {
        return `No content found in "${filters.category}" category`;
      }
      if (filters.tag !== 'all') {
        return `No content found with tag "${filters.tag}"`;
      }
  ...
  const getSuggestions = () => {
    ...
    return [
      ...
      isAuthenticated && 'Be the first to create content!',
    ].filter(Boolean);
  };
