...
import { getUniqueCategories } from '../../utils/sessionUtils';

/**
 * SessionFilters component with tab-based filtering, search, and category chips
 * @param {Object} props
 * @param {Object} props.filters - Current filter state
 * @param {Function} props.onFilterChange - Callback when filters change
 * @param {Array} props.sessions - All sessions for category extraction
 */
const SessionFilters = memo(({ filters, onFilterChange, sessions = [] }) => {
  const [searchQuery, setSearchQuery] = useState(filters.search || '');
  const [showCategories, setShowCategories] = useState(false);
  const categories = getUniqueCategories(sessions);
  ...
  const handleClearAll = useCallback(() => {
    setSearchQuery('');
    onFilterChange({ status: 'all', type: 'all', category: 'all', search: '' });
  }, [onFilterChange]);
